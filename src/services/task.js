...
export const addUser = function () {
    ...
    const updateUsersList = function (users) {
        ...
        users.forEach(user => {
            ...
            listItem.textContent = `ID: ${user.id} Login: ${user.login} Password: ${user.password}`;

            // Создаем кнопку "Remove User" для удаления пользователя
            const removeButton = document.createElement('button');
            removeButton.classList = "removeUser";
            removeButton.textContent = '- Remove User';
            removeButton.addEventListener('click', function () {
                removeUser(user.id, user.login, user.password);
            });
    ...
    const removeUser = function (id, login, password) {
        // Получаем текущих пользователей из локального хранилища
...
export const clickElement = function () {
    const elements = document.querySelectorAll('.kanban-element');
    const modal = document.getElementById('modal');
    const modalContent = modal.querySelector('.modal-content');
    const closeBtn = modal.querySelector('.close-btn');
    const taskDetails = modal.querySelector('#task-details');

    elements.forEach(element => {
        element.addEventListener('click', function () {
            // Отображаем модальное окно
            modal.style.display = 'block';
            // Очищаем содержимое модального окна перед отображением новой задачи
            taskDetails.innerHTML = '';
            // Клонируем содержимое задачи и добавляем его в модальное окно
            const clonedTask = element.cloneNode(true);
            taskDetails.appendChild(clonedTask);
        });
    });
